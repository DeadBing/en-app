...
import { contexts, readingGists, grammarVariants } from './contexts.js';
...
function nav() {
  return navItems.map(([id, glyph, label]) => `<button class="nav-item ${page === id ? 'active' : ''}" data-action="nav" data-page="${id}" ${page === id ? 'aria-current="page"' : ''}>${icon(glyph)}<span>${label}</span>${id === 'vocabulary' && s().due ? `<span class="nav-count">${s().due}</span>` : ''}</button>`).join('');
}
...
function vocabularyView() {
  ...
  return `${header('WORD BY WORD', 'Твой запас слов', `${allWords().length} слов и выражений · контекст вместо зубрёжки`, btn(`${icon('plus')} Своё слово`, 'add-word', 'outline'))}
    <div class="vocab-toolbar"><label class="search">${icon('search')}<input id="word-search" type="search" autocomplete="off" placeholder="Найти слово или перевод" value="${e(query)}" aria-label="Поиск слова или перевода"></label><button type="button" class="btn primary" data-action="review">${icon('repeat')} Практиковать подборку</button></div>
...
function wordExample(w, task) { return task.exampleIndex && contexts[w.term] ? contexts[w.term] : w.example; }
...
function sessionView() {
  ...
  return frame(`<h2>${g ? e(g.title) : r ? e(r.title) : task.mode === 'context' ? 'Какое выражение здесь подходит?' : 'Как сказать это по-английски?'}</h2>
